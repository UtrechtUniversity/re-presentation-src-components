...
- name: Example component
  ...
  tasks:
    ...
    - name: Output some more text
      ansible.builtin.debug:
        msg: Hostname contains web-
      when: hostname | regex_search('web-.*') # Jinja!
```
...
1. Python's `http` module's in-built fileserver (`python -m http.server`)
